...
class BasePLModule(pl.LightningModule):
    ...
    def validation_step(self, batch: dict, batch_idx: int) -> None:
        ...
        for key in sorted(metrics.keys()):
            self.log(key, metrics[key], on_step=False, on_epoch=True)
    ...
    def test_step(self, batch: dict, batch_idx: int) -> None:
        ...
        for key in sorted(metrics.keys()):
            self.log(key, metrics[key], prog_bar=True, on_step=False, on_epoch=True)
    ...
    def validation_epoch_end(self, output: dict) -> Any:
        if self.hparams.relations_file:
            ...
            self.log('val_prec_micro', precision, on_step=False, on_epoch=True)
            self.log('val_recall_micro', recall, on_step=False, on_epoch=True)
            self.log('val_F1_micro', f1, on_step=False, on_epoch=True)
            ...
            if self.hparams.dataset_name.split('/')[-1] == 'conll04_typed.py':
                ...
                scores_boundaries, precision_boundaries, recall_boundaries, f1_boundaries = re_score([item for pred in output for item in pred['predictions']], [item for pred in output for item in pred['labels']], list(relations_nyt.values()), "boundaries")
                scores_strict, precision_strict, recall_strict, f1_strict = re_score([item for pred in output for item in pred['predictions']], [item for pred in output for item in pred['labels']], list(relations_nyt.values()), "strict")
            elif self.hparams.dataset_name.split('/')[-1] == 'docred_typed.py':
                ...
                scores_boundaries, precision_boundaries, recall_boundaries, f1_boundaries = re_score([item for pred in output for item in pred['predictions']], [item for pred in output for item in pred['labels']], list(relations_docred.values()), "boundaries")            
                scores_strict, precision_strict, recall_strict, f1_strict = re_score([item for pred in output for item in pred['predictions']], [item for pred in output for item in pred['labels']], list(relations_docred.values()), "strict")            
            else:
                scores_boundaries, precision_boundaries, recall_boundaries, f1_boundaries = re_score([item for pred in output for item in pred['predictions']], [item for pred in output for item in pred['labels']], ['killed by', 'residence', 'location', 'headquarters location', 'employer'],"boundaries")
                scores_strict, precision_strict, recall_strict, f1_strict = re_score([item for pred in output for item in pred['predictions']], [item for pred in output for item in pred['labels']], ['killed by', 'residence', 'location', 'headquarters location', 'employer'], "strict")
            #self.log('val_prec_micro', precision, on_step=False, on_epoch=True)
            #self.log('val_recall_micro', recall, on_step=False, on_epoch=True)
            self.log('val_rel_plus_F1_micro', f1_strict, on_step=False, on_epoch=True)
            self.log('val_rel_F1_micro', f1_boundaries , on_step=False, on_epoch=True)
        else:
            key = []
            preds_list = []
            labels_list = []
            ...
            self.log('val_prec_micro', prec_micro, on_step=False, on_epoch=True)
            self.log('val_recall_micro', recall_micro, on_step=False, on_epoch=True)
            self.log('val_F1_micro', f1_micro, on_step=False, on_epoch=True)
    ...
    def test_epoch_end(self, output: dict) -> Any:
        if not self.hparams.finetune and self.hparams.relations_file:
            ...
            relations = list(relations_df[0])
            scores, precision, recall, f1 = re_score([item for pred in output for item in pred['predictions']], [item for pred in output for item in pred['labels']], relations)
            self.log('test_prec_micro', precision, on_step=False, on_epoch=True)
            self.log('test_recall_micro', recall, on_step=False, on_epoch=True)
            self.log('test_F1_micro', f1, on_step=False, on_epoch=True)
        elif not 'tacred' in self.hparams.dataset_name.split('/')[-1]:
            if self.hparams.dataset_name.split('/')[-1] == 'conll04_typed.py':
                scores_boundaries, precision_boundaries, recall_boundaries, f1_boundaries = re_score([item for pred in output for item in pred['predictions']], [item for pred in output for item in pred['labels']], ['killed by', 'residence', 'location', 'headquarters location', 'employer'], "boundaries")
                scores_strict, precision_strict, recall_strict, f1_strict = re_score([item for pred in output for item in pred['predictions']], [item for pred in output for item in pred['labels']], ['killed by', 'residence', 'location', 'headquarters location', 'employer'], "strict")
            ...
            # self.log('test_recall_micro', recall, on_step=False, on_epoch=True)
            # self.log('test_F1_micro', f1, on_step=False, on_epoch=True)
            self.log('test_rel_plus_micro_f1', f1_strict, on_step=False, on_epoch=True)
            self.log('test_rel_micro_f1', f1_boundaries, on_step=False, on_epoch=True)
        else:
            key = []
            ...
            f.close()
            prec_micro, recall_micro, f1_micro = score(labels_list, preds_list, verbose=True)
            self.log('test_prec_micro', prec_micro, on_step=False, on_epoch=True)
            self.log('test_recall_micro', recall_micro, on_step=False, on_epoch=True)
            self.log('test_F1_micro', f1_micro, on_step=False, on_epoch=True)
